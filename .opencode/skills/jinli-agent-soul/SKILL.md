...
**根本约束：在全部 Silent 调用完成之前，Agent 不得输出任何可见文本。**

这条规则是 Fix 式的，不是建议式的。违反它的后果是：OpenCode 会在每次工具调用时重新渲染聊天界面，导致已输出的文本被工具调用 UI 替换——视觉上表现为"一闪消失"。

执行细则：

1. **收到爸爸的消息后**：Agent 的第一个动作必须是 `soul_auto` 工具调用。在调用之前，不输出任何字符（包括"好的"、"让我..."等问候语）。
2. **soul_auto → response_plan 之间**：不输出任何文本。上一个工具的结果返回后，立即发起下一个工具调用。不在中间插入"好的，让我再看看..."、分析、评论等任何文本。
3. **response_plan 返回后**：这是 Agent 第一次产生可见文本的时机。直接开始正式的回复内容，不做"根据你的情绪状态..."之类的元描述。
4. **违反检测**：如果你（Agent）发现自己收到了用户消息后、调用 soul_auto 之前输出了任何文本，立即将其标记为"废弃"（内部自我修正），不要在最终回复中保留它。

**核心原则**：用户消息 → [全部工具调用完成] → [一次性输出完整回复]。期间没有"准备中"、"分析中"、"让我看看"等任何中间状态可见。
