...
description: "当前项目的 UE 需求路由智能体。先判断主链路、选择主 skill/次 skill、指定必读文档，再决定是否启用多智能体协作。已整合 Qoder 的 Plan 阶段完整流程 + 反降智协议。"
---
...
本 skill 是整个工作区的 UE 任务唯一入口。不负责直接实现功能，负责：
- 项目类型检测（UE5/Web/Other）
- 需求归类与主/次 skill 选择
- Plan 阶段完整流程（设计文档检索 → 隐含需求推导 → 依赖链推导 → 路由决策）
- 单 agent / 多 agent 判断
- 阶段状态管理
...
读取 `.trae/tasks/<task-name>/.task.yaml`，获取当前 phase 和 project_type。恢复规则：每次上下文恢复时重新执行 Step 0-1，不信任对话历史中的阶段信息。

### Step 3：阶段判定

按顺序检查：archived → verify_result → phase: verify → phase: review → phase: implement → phase: plan → 无活跃任务。
...
3. 读取最相关的 2-4 篇的关键章节
4. 执行隐含需求推导：从设计文档反向推导用户没说出口的前提条件

推导规则表：

| 设计文档声明了 | 用户没提但必然需要的 | 优先级 |
|--------------|-------------------|:---:|
| 某个数据模型/类型体系 | 新模块必须复用该类型，不能自创一组 | P0 |
...
- 如果设计文档中定义了模型/管线/校验/UI 约定，但隐含需求章节未覆盖 → 禁止进入依赖链推导

### 1c. 需求澄清

**REQUIRED SUB-SKILL:** Use `smart-requirements`.

先执行 `deep-discovery` / `fast-track` 分类。新系统、明显功能、工作流/UI 重构、架构或数据归属变化、高影响歧义必须深度访谈；只有精确、有限、无架构与完整用户旅程影响、没有待确认隐含需求且验证明确的改动才允许快速通道。无法判断时默认深度访谈。

深度访谈遵循：

- `one-question-per-turn`：每轮只问一个通俗问题
- 提供 2～3 个选择、推荐项和理由，允许自由回答
- `no-fixed-round-limit`：以重要不确定性是否清零为结束条件
- 每轮复述理解并展示隐含需求
- 结束前做 `teach-back` 需求回放并获得明确确认

确认后由 Plan Agent 自主编写 `requirements.md` 和 `execution-prompt.md`，再进入技术设计。原始用户消息不得直接作为执行模型的任务提示词。
...
搜索 2-3 个高质量项目，输出结构化对比摘要到 analysis.md。hotfix 工作流可跳过。

### 1f. 依赖链推导（强制）

从目标反向推导前提条件。每个 P0 依赖用户未提及 → 必须询问用户。
...
| 主 Skill | 适用场景 |
|----------|---------|
| `ue-lyra-gas-implementer` | Lyra+GAS+AI+单机玩法（已合并 ue57-lyra-gas-ai-singleplayer 和 lyra-gas-dev） |
...
- requirements.md：深度访谈后的通俗需求事实源
- execution-prompt.md：Plan Agent 根据确认需求编写的执行提示词
...
先确认通俗需求回放，再展示方案摘要：依赖链、隐式需求、架构方案、成熟方案证据、已拒绝的捷径、质量级别和验证方式。
...
- **Implement**：入口验证（can-edit）→ 加载主 Skill → 按 tasks.md 逐项实现 → 编译验证 → **每完成一个 Task 更新 spec-living 状态、changelog 和 doc-impact** → task-guard implement -Apply
- **Review**：code-quality-reviewer 独立审查 → task-guard review -Apply
- **Verify**：ue-ai-validator 验证收口 → 验收报告 → task-guard verify -Apply
- **Archive**：task-state transition archived
