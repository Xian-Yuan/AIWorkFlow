...
```
1. Docs/APIRef/CommonPatterns.md    → 10 种经实战验证的代码模式 (ASC/伤害/属性/事件/目标/GameplayCue)
2. Docs/APIRef/UEMacrosRef.md       → UE 宏全谱 (反射/复制/RPC/Blueprint 互操作)
3. Docs/Troubleshooting/ErrorKnowledgeBase/ → 29 条错误条目 (编译/运行时/资产/逻辑)
4. Docs/APIRef/UECommonAPIRef.md    → UGameplayStatics/KismetSystemLibrary 高频 API
5. Docs/CodeTemplates/              → 完整可编译模板 (GameFeature/GA/GE/AS 等)
```
...
- **先检索 CommonPatterns.md**, 确认目标模式是否已有现成模板。
...
遇到编译/运行时错误时：
1. **先查 ErrorKB**: `Docs/Troubleshooting/ErrorKnowledgeBase/` 用错误关键词模糊匹配
2. 找到匹配 → 按已知方案修复
3. 未找到 → 修复后新建条目 (E000-TEMPLATE.md)
...
- Symptom: class compiles but is missing from Blueprint.
  ...
  - ErrorKB: E005-Compile-DeriveWithoutUCLASS
- Symptom: unresolved symbol or include cycles.
  ...
  - ErrorKB: E002-Compile-ModuleDependency, E003-Compile-IncludePath
